...
//        $request = cache($key, $value, $this->tokenExpire);//第三参数是时效期
    ...
    public function SaveCloudToken($token)
    {
        $res=Cache::set('otoken',$token,$this->tokenExpire);
        if (!$res){
            throw new TokenException(['msg' => '服务器缓存异常']);
        }
        return $res;
    ...
    {
        $res=Cache::get('otoken');
        return $res;
    }

    public function get_sin_token($stoken)
    {
        $get=(new BaseCommon())->curl_sa_post($stoken,WEB_URL.'/Verify');
