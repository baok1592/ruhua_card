...
class Index extends BaseController
{
    protected function initialize()
    {
        ...
        // 检测是否安装过
        $fiel=file_exists(VAE_ROOT.'/data/install.lock');
    ...
    public function createData()
    {
        ...
        if ($this->request->isPost()) {
            ...
            // 导入sql数据并创建表
            $vaethink_sql = file_get_contents(VAE_ROOT . '/data/card.sql');
            $sql_array = preg_split("/;[\r\n]+/", str_replace("ims_qy2021_order_", $data['DB_PREFIX'], $vaethink_sql));
...
//            //插入管理员
//            $post['name'] = input('post.username');
//            $post['pwd'] = md5(input('post.password'));
//            $if=admin::where('name',$post['name'])->find();
//            if($if!='')
...
//            }
            ...
            $api_url=Request::domain();
            $sql="update {$data['DB_PREFIX']} set value='{$api_url}' where `key`='api_url'";
            $link->query($sql);
